...
use module_namelist, only: sam_default_seed
use tools_display, only: ddis
...
interface
   subroutine initialize_sampling_c(randgen,ddis,n,lon,lat,mask,L,ntry,nrep,ns,nfor,ifor,ihor) bind(C,name="initialize_sampling")
   use iso_c_binding
   implicit none
   type(c_ptr),value :: randgen
   integer(c_int),value :: ddis
...
real,intent(in) :: lon(n)             !< Grid points longitude
real,intent(in) :: lat(n)             !< Grid points latitude
integer,intent(in) :: mask(n)         !< Grid mask
real,intent(in) :: L(n)               !< Grid length-scale
...
if (mpl%main) then
   ! Call C++ function
   call initialize_sampling_c(this%ptr,ddis,n,lon,lat,mask,L,ntry,nrep,ns,nfor,ifor,ihor)
end if
