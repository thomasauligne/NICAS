...
use module_namelist, only: datadir,prefix,model,levs,ntry,nrep,lsqrt,Lbh_file,Lbh,Lbv_file,Lbv,resol,network
...
integer :: ncid,nc1_id,two_id,namax_id,nl1_id,nc2_id,lon_id,lat_id,na_id,larc_id
real :: distnorm
real :: rh0(sdata%nc0,sdata%nl0),rv0(sdata%nc0,sdata%nl0),rh0min(sdata%nc0)
real,allocatable :: rh1(:,:),rv1(:,:),rh2(:,:),rv2(:,:),rhs(:),rvs(:)
character(len=1024) :: filename
character(len=1024) :: subr = 'compute_parameters'
...
! Compute adaptive mesh
write(mpl%unit,'(a7,a)') '','Compute adaptive mesh'
...
if (trim(Lbh_file)=='') then
   ...
   call model_read(trim(Lbh_file),'Lh',sdata,rh0)
end if
if (trim(Lbv_file)=='') then
   ...
   call model_read(trim(Lbv_file),'Lv',sdata,rv0)
end if
write(mpl%unit,'(a10,a,e10.3,a,e10.3)') '','Average length-scales (H/V): ',sum(rh0)/float(sdata%nc0*sdata%nl0),' / ',sum(rv0)/float(sdata%nc0*sdata%nl0)

! Compute scaled support radii
rh0 = 2.0*rh0/(sqrt(0.3)*req)
rv0 = 2.0*rv0/sqrt(0.3)
write(mpl%unit,'(a10,a,e10.3,a,e10.3)') '','Average scaled support radii (H/V): ',sum(rh0)/float(sdata%nc0*sdata%nl0),' / ',sum(rv0)/float(sdata%nc0*sdata%nl0)

! Basic horizontal mesh defined with the minimum support radius
rh0min = minval(rh0,dim=2)
sdata%nc1 = floor(2.0*maxval(sdata%area)*resol**2/(sqrt(3.0)*(sum(rh0min,mask=isnotmsr(rh0min))/float(count(isnotmsr(rh0min))))**2))+sdata%nfor
mask_cell = 0
do ic0=1,sdata%nc0
   if (any(sdata%mask(ic0,:))) mask_cell(ic0) = 1
end do
allocate(sdata%ic1_to_ic0(sdata%nc1))
call initialize_sampling(sdata%rng,sdata%nc0,dble(sdata%lon),dble(sdata%lat),mask_cell,rh0min,ntry,nrep,sdata%nc1,sdata%nfor,sdata%ifor,sdata%ic1_to_ic0)
...
! Vertical sampling
write(mpl%unit,'(a7,a)',advance='no') '','Vertical sampling: '
...
do il0=1,sdata%nl0
   ...
   if ((il0==1).or.(il0==sdata%nl0)) then
      ...
      sdata%llev(il0) = distnorm>1.0/resol
   end if
...
do il0=1,sdata%nl0
   if (sdata%llev(il0)) then
      write(mpl%unit,'(i3,a)',advance='no') levs(il0),' '
      il1 = il1+1
...
do il1=1,sdata%nl1
   il0 = sdata%il1_to_il0(il1)
   sdata%nc2(il1) = floor(2.0*sdata%area(il0)*resol**2/(sqrt(3.0)*(sum(rh2(:,il1),mask=isnotmsr(rh2(:,il1)))/float(count(isnotmsr(rh2(:,il1)))))**2))+sdata%nfor
   ...
   if (sdata%nc2(il1)<sdata%nc1) then
      ...
      call initialize_sampling(sdata%rng,sdata%nc1,dble(sdata%lon(sdata%ic1_to_ic0)),dble(sdata%lat(sdata%ic1_to_ic0)),mask_cell(sdata%ic1_to_ic0),rh0(sdata%ic1_to_ic0,il0),ntry,nrep,sdata%nc2(il1),sdata%nfor,ifors,sdata%ic2il1_to_ic1(1:sdata%nc2(il1),il1))
   else
...
if (mpl%main) then
   ! Write mesh
   write(mpl%unit,'(a7,a)') '','Write subgrid mesh'
   filename = trim(prefix)//'_mesh.nc'
   call ncerr(subr,nf90_create(trim(datadir)//'/'//trim(filename),or(nf90_clobber,nf90_64bit_offset),ncid))
   call ncerr(subr,nf90_def_dim(ncid,'nc1',sdata%nc1,nc1_id))
   call ncerr(subr,nf90_def_dim(ncid,'two',2,two_id))
   call ncerr(subr,nf90_def_dim(ncid,'namax',maxval(sdata%na),namax_id))
   call ncerr(subr,nf90_def_dim(ncid,'nl1',sdata%nl1,nl1_id))
   call ncerr(subr,nf90_def_var(ncid,'nc2',nf90_int,(/nl1_id/),nc2_id))
   call ncerr(subr,nf90_def_var(ncid,'lon',ncfloat,(/nc1_id,nl1_id/),lon_id))
   call ncerr(subr,nf90_def_var(ncid,'lat',ncfloat,(/nc1_id,nl1_id/),lat_id))
   call ncerr(subr,nf90_def_var(ncid,'na',nf90_int,(/nl1_id/),na_id))
   call ncerr(subr,nf90_def_var(ncid,'larc',nf90_int,(/two_id,namax_id,nl1_id/),larc_id))
   call ncerr(subr,nf90_enddef(ncid))
   call ncerr(subr,nf90_put_var(ncid,nc2_id,sdata%nc2))
   call ncerr(subr,nf90_put_var(ncid,na_id,sdata%na))
   do il1=1,sdata%nl1
      call ncerr(subr,nf90_put_var(ncid,lon_id,sdata%lon(sdata%ic2il1_to_ic0(1:sdata%nc2(il1),il1))*rad2deg,(/1,il1/),(/sdata%nc2(il1),1/)))
      call ncerr(subr,nf90_put_var(ncid,lat_id,sdata%lat(sdata%ic2il1_to_ic0(1:sdata%nc2(il1),il1))*rad2deg,(/1,il1/),(/sdata%nc2(il1),1/)))
      call ncerr(subr,nf90_put_var(ncid,larc_id,sdata%larc(:,1:sdata%na(il1),il1),(/1,1,il1/),(/2,sdata%na(il1),1/)))
   end do
   call ncerr(subr,nf90_close(ncid))
   call model_write(filename,'rh',sdata,rh0*req)
   call model_write(filename,'rv',sdata,rv0)
end if
...
do il1=1,sdata%nl1
   write(mpl%unit,'(a10,a,i2,a,i8)') '','nc2(',il1,') =   ',sdata%nc2(il1)
end do
write(mpl%unit,'(a10,a,i8)') '','ns =        ',sdata%ns
do il0i=1,sdata%nl0i
   write(mpl%unit,'(a10,a,i2,a,i8)') '','h(',il0i,')%n_s = ',sdata%h(il0i)%n_s
end do
do il0i=1,sdata%nl0i
   write(mpl%unit,'(a10,a,i2,a,i8)') '','v(',il0i,')%n_s = ',sdata%v(il0i)%n_s
end do
do il1=1,sdata%nl1
   write(mpl%unit,'(a10,a,i2,a,i8)') '','s(',il1,')%n_s = ',sdata%s(il1)%n_s
end do
...
real :: latbnd(2),lonbnd(2),v1(3),v2(3)
real,allocatable :: x(:),y(:),z(:),dist(:)
logical :: init

! Look for redundant or masked points
call msi(redundant)
do ic0=1,sdata%nc0
...
do ic0=1,sdata%nc0
   do i=1,sdata%grid_nnb(ic0)
      call sphere_dist(sdata%lon(ic0),sdata%lat(ic0),sdata%lon(sdata%grid_inb(i,ic0)),sdata%lat(sdata%grid_inb(i,ic0)),sdata%grid_dnb(i,ic0))
...
do ic1=1,sdata%nc1
   do i=1,sdata%subgrid_nnb(ic1)
      call sphere_dist(sdata%lon(ic1),sdata%lat(ic1),sdata%lon(sdata%subgrid_inb(i,ic1)),sdata%lat(sdata%subgrid_inb(i,ic1)),sdata%subgrid_dnb(i,ic1))
...
do il1=1,sdata%nl1
   ...
   ! Transform to cartesian coordinates
   call trans(sdata%nc2(il1),sdata%lat(sdata%ic2il1_to_ic0(1:sdata%nc2(il1),il1)),sdata%lon(sdata%ic2il1_to_ic0(1:sdata%nc2(il1),il1)),x,y,z)
