...
use module_namelist, only: datadir,model,nl,prefix,lsqrt,nproc,mpicom,namncwrite
use netcdf
...
type sdatatype
   ...
   real,allocatable :: area(:)             !< Domain area

   ! Vector coordinates
   real,allocatable :: lon(:)              !< Cells longitude
   real,allocatable :: lat(:)              !< Cells latitude
   logical,allocatable :: mask(:,:)        !< Cells mask
   real,allocatable :: vunit(:)            !< Vertical unit
   ...
   real,allocatable :: grid_dnb(:,:)       !< Neighbors distances on the full grid
   ...
   real,allocatable :: subgrid_dnb(:,:)    !< Neighbors distances on the subgrid

   ! Boundary nodes
   integer,allocatable :: nbnd(:)          !< Number of boundary nodes
   real,allocatable :: xbnd(:,:,:)         !< Boundary nodes, x-coordinate
   real,allocatable :: ybnd(:,:,:)         !< Boundary nodes, y-coordinate
   real,allocatable :: zbnd(:,:,:)         !< Boundary nodes, z-coordinate
   real,allocatable :: vbnd(:,:,:)         !< Boundary nodes, orthogonal vector
   ...
   ! Mask issues conversion
   integer,allocatable :: ic0(:)             !< Indices reordering
...
integer :: vbot_id,nc2_id,is_to_ic1_id,is_to_il1_id,is_to_ic2_id,ic0il0_to_is_id,ic2il1_to_ic0_id,ic2il1_to_is_id,ic1_to_ic0_id,il1_to_il0_id,ic0_to_ic1_id,il0_to_il1_id,ic2il1_to_ic1_id,ic1il1_to_is_id,norm_id,norm_sqrt_id
character(len=1024) :: filename
character(len=1024) :: subr = 'sdata_read_param'

! Open file and get dimensions
filename = trim(prefix)//'_param.nc'
...
if (lsqrt) allocate(sdata%norm_sqrt%val(sdata%ns))
...
if (lsqrt) call ncerr(subr,nf90_inq_varid(ncid,'norm_sqrt',norm_sqrt_id))
...
if (lsqrt) call ncerr(subr,nf90_get_var(ncid,norm_sqrt_id,sdata%norm_sqrt%val))
...
elseif (sdata%nproc>1) then
   ...
   ! Open file
   filename = trim(prefix)//'_distribution_'//nprocchar//'.nc'
   call ncerr(subr,nf90_open(trim(datadir)//'/'//trim(filename),nf90_nowrite,ncid))
...
do iproc=1,sdata%nproc
   ...
   filename = trim(prefix)//'_mpi-'//mpicomchar//'_'//nprocchar//'-'//iprocchar//'.nc'
   call ncerr(subr,nf90_open(trim(datadir)//'/'//trim(filename),nf90_nowrite,ncid))
...
if (mpl%main) then
   ...
   if (lsqrt) call ncerr(subr,nf90_def_var(ncid,'norm_sqrt',ncfloat,(/ns_id/),norm_sqrt_id))
   ...
   if (lsqrt) call ncerr(subr,nf90_put_var(ncid,norm_sqrt_id,sdata%norm_sqrt%val))
...
if (mpl%main) then
   ...
   write(mpicomchar,'(i1)') sdata%mpicom
   write(nprocchar,'(i4.4)') sdata%nproc
   filename = trim(prefix)//'_mpi-'//mpicomchar//'_'//nprocchar//'_summary.nc'
   call ncerr(subr,nf90_create(trim(datadir)//'/'//trim(filename),or(nf90_clobber,nf90_64bit_offset),ncid))
