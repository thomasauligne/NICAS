...
!
  if ( s12 == 0.0 .or. s23 == 0.0 .or. s31 == 0.0 ) then
    areas = 0.0
    return
  end if
...
!    deletion.
!
!    Input/output, real X(N), Y(N), Z(N), the coordinates of
...
!
      if ( left ( x1, y1, z1, x2, y2, z2, x(nl), y(nl), z(nl) ) ) then
        ...
        if ( ( 0.0 <= dp2l - dp12 * dp1l .or. &
               0.0 <= dp2r - dp12 * dp1r )  .and. &
             ( 0.0 <= dp1l - dp12 * dp2l .or.  &
               0.0 <= dp1r - dp12 * dp2r ) ) then
          go to 6
        end if
!
...
!
!    Output, real DF, value of an increasing function (negative
...
!  Parameters:
!
!    Input, real P(3), the coordinates of the point (unit vector)
!    to be located.
!
!    Input, integer LV, the length of arrays XV, YV, and ZV.
!
!    Input, real XV(LV), YV(LV), ZV(LV), the coordinates of unit
...
!
  ...
  lft2 = 0.0 < cn(1) * xv(i2) + cn(2) * yv(i2) + cn(3) * zv(i2)
!
...
!  <B,QN> > 0 and 0 < <B,PN>.
!
    if ( 0.0 < dot_product ( b(1:3), qn(1:3) ) .and. &
         0.0 < dot_product ( b(1:3), pn(1:3) ) ) then
!
...
!
!    Input, real CN(3), the coordinates of a nonzero vector
...
!
!    Output, real P(3), point of intersection defined above
...
!
!    Output, real STORE, the value of X after it has been stored
...
!
!    Output, real X(N), Y(N), Z(N), the coordinates in the
...
!
  ...
  do i = 1, nn
    if ((abs(rlat(i)+999.0)>tiny(1.0)).and.(abs(rlon(i)+999.0)>tiny(1.0))) then
      phi = rlat(i)
      theta = rlon(i)
      cosphi = cos ( phi )
...
!
!    Input, real P(3), the x, y, and z coordinates (in that order)
!    of the point P to be located.
!
!    Input, integer N, the number of nodes in the triangulation.
!    3 <= N.
!
!    Input, real X(N), Y(N), Z(N), the coordinates of the
!    triangulation nodes (unit vectors).
!
!    Input, integer LIST(6*(N-2)), LPTR(6*(N-2)), LEND(N), the
!    data structure defining the triangulation, created by TRMESH.
!
!    Output, real B1, B2, B3, the unnormalized barycentric
...
3   continue
    ...
    if ( det ( x(n0),y(n0),z(n0),x(n1),y(n1),z(n1),xp,yp,zp ) < 0.0 ) then
      lp = lptr(lp)
      n1 = list(lp)
      if ( n1 == nl ) then
        go to 6
...
!
    do
      ...
      if ( det(x(n1),y(n1),z(n1),x(n0),y(n0),z(n0),xp,yp,zp) < 0.0 ) then
        exit
      end if
...
!
    if ( det ( x(n0),y(n0),z(n0),x(n4),y(n4),z(n4),xp,yp,zp ) < 0.0 ) then
      n3 = n2
      n2 = n4
      n1s = n1
...
10 continue
  ...
  if ( det(x(n2),y(n2),z(n2),x(next),y(next),z(next),xp,yp,zp) >= 0.0 ) then
!
...
!    3 <= N.
!
!    Input, real X(N), Y(N), Z(N), the coordinates of distinct
...
!
!    Input, real PLTSIZ, the plot size in inches.  A circular
...
!
!    Input, real ELAT, ELON, the latitude and longitude
...
!    180, respectively.
!
!    Input, real A, the angular distance in degrees from E
...
!
  ...
  if ( 90.0 < abs ( elat ) ) then
    ier = 2
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'TRPLOT - Fatal error!'
    write ( *, '(a)' ) '  90 < |ELAT|!'
    return
  else if ( 180.0 < abs ( elon ) ) then
    ier = 2
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'TRPLOT - Fatal error!'
    write ( *, '(a)' ) '  180 < |ELON|!'
...
!
!    Input, real ELAT, ELON, the latitude and longitude (in
...
!
!    Input, real A, the angular distance in degrees from E to the
...
!    K for K = 1 to N.
!
!    Input, real XC(NT), YC(NT), ZC(NT), the coordinates of the
...
!
  ...
  if ( pltsiz < 1.0 .or. 8.5 < pltsiz .or. &
...
!
    in2 = ( 0.0 <= z2 ) .and. ( x2 * x2 + y2 * y2 <= wrs )
!
