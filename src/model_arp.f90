...
! Local variables
integer :: nc0,ilon,ilat,il0,ic0
integer :: ncid,nlon_id,nlat_id,nlev_id,lon_id,lat_id,a_id,b_id
integer,allocatable :: ic0_vec(:),ic0_tab(:,:)
...
! Conversion array
allocate(ic0_vec(sdata%nc0))
allocate(ic0_tab(sdata%nlon,sdata%nlat))
do ic0=1,sdata%nc0
   ic0_vec(ic0) = ic0
end do
call msi(ic0_tab)
ic0_tab = unpack(ic0_vec,mask=sdata%rgmask,field=ic0_tab)
allocate(sdata%ic0(nc0))
sdata%ic0 = pack(ic0_tab,mask=.true.)

! Pack
call sdata_alloc(sdata)
sdata%lon = pack(real(lon,kind(1.0)),mask=sdata%rgmask)
sdata%lat = pack(real(lat,kind(1.0)),mask=sdata%rgmask)
sdata%mask = .true.
...
! Pack data
do il0=1,sdata%nl0
   fld(:,il0) = pack(real(fld_loc(:,:,il0),kind(1.0)),mask=sdata%rgmask)
end do
