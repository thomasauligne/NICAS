...
use module_namelist, only: lsqrt
use omp_lib
...
integer,allocatable :: ineh(:,:),interp_h(:,:,:),inev(:,:),interp_v(:,:,:),ines(:,:),interp_s(:,:,:),inec(:),convol_c(:,:),order(:),is_list(:)
real :: S_add
real,allocatable :: interp_h_S(:,:,:),interp_v_S(:,:,:),interp_s_S(:,:,:),convol_c_S(:,:),S_list(:),S_list_tmp(:),norm(:),normg(:,:)
logical :: conv
...
do il0=1,sdata%nl0
   ...
   !$omp parallel do private(ic0_loc,ic0,ibuf,is_list,order,S_list,S_list_tmp,valid_list_tmp,nlr,is_add,S_add,ih,ic1,iv,il1,is,ilr,conv,ic,jlr,js)
   do ic0_loc=1,nc0_loc(mpl%myproc)
      ...
      if (sdata%mask(ic0,il0)) then
         ...
         deallocate(S_list)
         deallocate(S_list_tmp)
         if (lsqrt) deallocate(valid_list_tmp)
      end if
   end do
   !$omp end parallel do
end do
