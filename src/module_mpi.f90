...
use module_namelist, only: datadir,nl,levs,ntry,nrep,Lbh,Lbv,resol,nproc
use netcdf
use omp_lib
...
do iproc=1,sdata%nproc
   ...
   do il1=1,sdata%mpi(iproc)%nl1
      write(mpl%unit,'(a10,a,i2,a,i8)') '','nc2b(',il1,') =  ',sdata%mpi(iproc)%nc2b(il1)
   end do
   ...
   do il0i=1,sdata%mpi(iproc)%nl0i
      write(mpl%unit,'(a10,a,i2,a,i8)') '','h(',il0i,')%n_s = ',sdata%mpi(iproc)%h(il0i)%n_s
   end do
   do il1=1,sdata%mpi(iproc)%nl1
      write(mpl%unit,'(a10,a,i2,a,i8)') '','s(',il1,')%n_s = ',sdata%mpi(iproc)%s(il1)%n_s
   end do
...
do jproc=1,sdata%nproc
   ...
   do iproc=1,sdata%nproc
      do i=1,sdata%mpi(iproc)%AB%jhalocounts(jproc)
         ...
         sdata%mpi(jproc)%AB%excl(sdata%mpi(jproc)%AB%jexcldispl(iproc)+i) = sdata%mpi(iproc)%AB%halo(sdata%mpi(iproc)%AB%jhalodispl(jproc)+i)
      end do
      do i=1,sdata%mpi(iproc)%AC%jhalocounts(jproc)
         ! Global index of points received on JPROC from IPROC
         sdata%mpi(jproc)%AC%excl(sdata%mpi(jproc)%AC%jexcldispl(iproc)+i) = sdata%mpi(iproc)%AC%halo(sdata%mpi(iproc)%AC%jhalodispl(jproc)+i)
      end do
   end do
end do
