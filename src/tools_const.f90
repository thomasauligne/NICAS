...
real,parameter :: deg2rad=pi/180.0 !< Degree to radian
real,parameter :: rad2deg=180.0/pi !< Radian to degree
real,parameter :: req=6.371e6      !< Earth radius
real,parameter :: epsilon=1.0e-12  !< Small parameter
real,parameter :: ps=101325.0      !< Reference surface pressure
...
! Passed variable
real,intent(in) :: lon_i !< Initial point longitude (radian)
real,intent(in) :: lat_i !< Initial point latitude (radian)
real,intent(in) :: lon_f !< Final point longitude (radian)
real,intent(in) :: lat_f !< Final point longilatitudetude (radian)
real,intent(out) :: dist !< Great-circle distance
...
if (isnotmsr(lon_i).and.isnotmsr(lat_i).and.isnotmsr(lon_f).and.isnotmsr(lat_f)) then
   ! Great-circle distance using Vincenty formula on the sphere
    dist = req*atan2(sqrt((cos(lat_f)*sin(lon_f-lon_i))**2+(cos(lat_i)*sin(lat_f)-sin(lat_i)*cos(lat_f)*cos(lon_f-lon_i))**2),sin(lat_i)*sin(lat_f)+cos(lat_i)*cos(lat_f)*cos(lon_f-lon_i))
else
   call msr(dist)
end if
...
real,intent(in) :: v1(3) !< First vector
real,intent(in) :: v2(3) !< Second vector
real,intent(in) :: v3(3) !< Third vector
real,intent(out) :: p    !< Triple product
