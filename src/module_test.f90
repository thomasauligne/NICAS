...
use module_namelist, only: datadir,prefix,model,levs,ndir,dirlon,dirlat,lsqrt,nproc
use omp_lib
...
private
public :: test_adjoints,test_pos_def,test_mpi,test_dirac
...
write(mpl%unit,'(a7,a,e14.8,a,e14.8,a,e14.8)') '','Interpolation adjoint test: ',sum1,' / ',sum2,' / ',2.0*abs(sum1-sum2)/abs(sum1+sum2)
...
write(mpl%unit,'(a7,a,e14.8,a,e14.8,a,e14.8)') '','Convolution adjoint test:   ',sum1,' / ',sum2,' / ',2.0*abs(sum1-sum2)/abs(sum1+sum2)
...
if (lsqrt) then
   ...
   write(mpl%unit,'(a7,a,e14.8,a,e14.8,a,e14.8)') '','NICAS square-root adjoint test: ',sum1,' / ',sum2,' / ',2.0*abs(sum1-sum2)/abs(sum1+sum2)
else
   ! Allocation
   ...
   write(mpl%unit,'(a7,a,e14.8,a,e14.8,a,e14.8)') '','NICAS adjoint test:         ',sum1,' / ',sum2,' / ',2.0*abs(sum1-sum2)/abs(sum1+sum2)
end if
...
integer :: iter
real :: norm,egvmax,egvmax_prev,egvmin,egvmin_prev
...
! Non conclusive test
if (iter==nitermax+1) write(mpl%unit,'(a7,a,e14.8,a,i4,a,e14.8)') '','NICAS seems to be positive definite: difference ',egvmax+egvmin,' after ',nitermax,' iterations for a tolerance ',tol
...
! Print difference
write(mpl%unit,'(a7,a,e14.8)') '','RMSE between single-proc and multi-procs execution: ',sqrt(sum((flds%val-fldm%val)**2)/float(sdata%nc0*sdata%nl0))
...
allocate(distmin(ndir))
allocate(ic0_dir(ndir))
distmin = huge(1.0)
do ic0=1,sdata%nc0
   if (sdata%mask(ic0,1)) then
      do idir=1,ndir
         call sphere_dist(dirlon(idir)*deg2rad,dirlat(idir)*deg2rad,sdata%lon(ic0),sdata%lat(ic0),dist)
...
do il0=1,sdata%nl0
   ! Remove files
   write(levchar,'(i3.3)') levs(il0)
   call system('rm -f '//trim(datadir)//'/'//trim(prefix)//'_dirac_'//levchar//'.nc')
   ...
   ! Write field
   call model_write(trim(prefix)//'_dirac_'//levchar//'.nc','hr',sdata,dirac%val)

   ! Print results
   write(mpl%unit,'(a7,a)') '','Normalization at level '//levchar
   do idir=1,ndir
      write(mpl%unit,'(a10,f6.1,a,f6.1,a,f10.7)') '',dirlon(idir),' / ',dirlat(idir),': ',dirac%val(ic0_dir(idir),il0)
   end do
   write(mpl%unit,'(a7,a,f10.7,a,f10.7)') '','Min - max at level '//levchar//': ',minval(dirac%val(:,il0),mask=sdata%mask(:,il0)),' - ',maxval(dirac%val(:,il0),mask=sdata%mask(:,il0))
end do
